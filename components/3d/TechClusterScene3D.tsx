...
const techs: TechItem[] = [
  { name: 'OpenAI Agents SDK', category: 'AI & Agentic', color: '#9463c2' },
  { name: 'MCP', category: 'AI & Agentic', color: '#b07cd8' },
  { name: 'RAG', category: 'AI & Agentic', color: '#9463c2' },
...
function GroundPlane() {
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.6, 0]}>
      ...
      <meshBasicMaterial color="#9463c2" opacity={0.03} transparent side={THREE.DoubleSide} />
    </mesh>
  );
}
...
function SceneInner({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return (
    <>
      ...
      <pointLight position={[4, 6, 4]} intensity={0.5} color="#9463c2" />
      <pointLight position={[-3, 2, -5]} intensity={0.3} color="#5b21b6" />
      <directionalLight position={[2, 3, -6]} intensity={0.5} />
      <directionalLight position={[-2, 1, 5]} intensity={0.2} color="#9463c2" />
